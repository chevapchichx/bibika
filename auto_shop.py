...
def main_window():
    ...
    auto_go_btn = Button(window_main, text="Выбрать бибику", command=open_bibiki_contents_window, fg="#6E7B8B",
                         font=("comic sans", 16), relief="flat", borderwidth=0, )
    canvas.create_window((90, 500), anchor="nw", window=auto_go_btn)

    manager_go_btn = Button(window_main, text="Личный кабинет", command=lambda: auto_manager.auth_lk_window(
        lambda login, password: add_info_man_button(window_main, canvas, login, password), add_bibiki_change_button,
        window_main, canvas), fg="#6E7B8B", font=("comic sans", 11), relief="flat", borderwidth=0)
